...
class GFCnpEcheckField {
	...
	*/
	public static function getPost($field_id) {
		...
		if($form_currency == "CAD"){$routing = $echeck['routing']."".$echeck['transit'];}else{$routing = $echeck['routing'];}
		if (is_array($echeck)) {
			$echeck = array (
				'routing' => $routing,
				'check' => $echeck['check'],
				...
				if (!is_array($value)) {
					...
					if($formData2->creditcardCount == 0)
					{
						...
						if (empty($value['routing'])) {
							if($form_currency == "CAD"){
								$messages[] = "Please enter Bank Number & Transit Number.";
							}
							else
							{
	...
	*/
	public function gformFieldInput($input, $field, $value, $lead_id, $form_id) {
		...
		if ($field['type'] == GFCNP_FIELD_ECHECK) {
			...
			$input .= "Check Number <span class='gfield_required'>*</span>: <input type='text' name='gfp_{$field['id']}[check]' id='gfp_{$field['id']}_check'  value='{$echeck['check']}' class='echeck_check'$isrecurring$disabled_text maxlength=10><br>";
			$SavingsAccount = '';
			$CheckingAccount = '';
			...
			if($form_currency == "CAD"){ $dsplytext = "Bank Number";}else{$dsplytext = "Routing Number";}
			if($form_currency == "CAD"){ $dsplymaxtext = "3";}else{$dsplymaxtext = "9";}
			$input .= $dsplytext."<span class='gfield_required'>*</span>: <input type='text' name='gfp_{$field['id']}[routing]' id='gfp_{$field['id']}'  value='{$echeck['routing']}' class='echeck_routing'$isrecurring$disabled_text maxlength='".$dsplymaxtext."'><br>";
			if($form_currency == "CAD"){
			$input .= "Transit Number <span class='gfield_required'>*</span>: <input type='text' name='gfp_{$field['id']}[transit]' id='gfp_{$field['id']}_transit'  value='{$echeck['transit']}' class='echeck_transit'$isrecurring$disabled_text maxlength=5><br>";
			}
			
			
			$input .= "Account Number <span class='gfield_required'>*</span>: <input type='text' name='gfp_{$field['id']}[account]' id='gfp_{$field['id']}_account'  value='{$echeck['account']}' class='echeck_account'$isrecurring$disabled_text maxlength=17><br>";
			
			$input .= "Repeat Account Number <span class='gfield_required'>*</span>: <input type='text' name='gfp_{$field['id']}[reaccount]' id='gfp_{$field['id']}_account'  value='{$echeck['reaccount']}' class='echeck_account'$isrecurring$disabled_text maxlength=17><br>";
