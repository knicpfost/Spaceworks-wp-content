...
class GFCnpFormData {
	...
	public $paymentfieldCount = 0;
	public $inc = 0;
	private $isLastPageFlag = FALSE;
	private $isOrderModeHiddenFlag = FALSE;
	...
	*/
	private function loadForm(&$form) {
		...
		foreach ($form['fields'] as &$field) {
			if (RGFormsModel::get_input_type($field) == 'singleshipping') $shipp_adrss = TRUE;
		}

		foreach ($form['fields'] as &$field) {
			$id = $field['id'];
			$checkbox_array = $multiselect_array = $select_array = $list_array = array();
			//echo RGFormsModel::get_input_type($field).'<br>';
			//if (RGFormsModel::get_input_type($field) == 'singleshipping') $shipp_adrss = TRUE; //else $this->shipp_adrss = FALSE; 
			//echo $shipp_adrss;
			switch(RGFormsModel::get_input_type($field)){
					...
					if (empty($this->email)) {
						...
						/*if($item_custom['FieldValue'])
						{
							if(count($item_custom)) {
								...
								//echo $hasfield.'--'.$cfield['FieldId'].'+++'.$item_custom['FieldId'];
								if($hasfield)
									$this->customfields[] = $item_custom;
							}
						}*/
				...
				case 'address':
					...
					elseif(empty($this->address_shipping) && empty($this->postcode_shipping) && $shipp_adrss) 
					{
					...
					else
					{
						...
						$parts = array($str, trim(rgpost("input_{$id}_3")), trim(rgpost("input_{$id}_4")), trim(rgpost("input_{$id}_5")),trim(rgpost("input_{$id}_6")));
						$str2 = implode(', ', array_filter($parts, 'strlen'));
						
						$item_custom['FieldValue'] = $str1.','.$str2;//implode(', ', array_filter($str2, 'strlen'));
						$item_custom['FieldId'] = $id;
						$this->customfields[] = $item_custom;
						/*if($item_custom['FieldValue'])
						{
				...
				case 'gfcnpecheck':
					...
					if($this->isEcheckHiddenFlag == FALSE){
					$this->ecField =& $field;
					$echeck = rgpost('gfp_' . $id);
					if($echeck['transit'] == ""){$this->ecRouting = $echeck['routing'];}else {$this->ecRouting = $echeck['routing'].$echeck['transit'];}
				...
				case 'gfcnpformmode': 
					...
					$this->isOrderModeHiddenFlag = RGFormsModel::is_field_hidden($form, $field, RGForms::post('gform_field_values'));
				
					if($this->isOrderModeHiddenFlag == FALSE){
					
					$this->ordermodeField =& $field;
				
					$this->ordermode = rgpost('gfcnp_formmode_hid');
				...
				default:
					...
					if (GFCommon::is_product_field($field['type']) || $field['type'] == 'donation') {
						$prdprice = self::getProductPrice($form, $field);
					...
					//else
					{
						switch($field['type'])
						{
							case 'checkbox':								
								...
								/*if ((strtolower($field['label']) == 'payment method' || strtolower($new_arry['label']) == 'select payment type'  || 
							     strtolower($new_arry['label']) == 'select payment method') && $this->inc == 1)
								 {print_r($item_custom); 
									$this->paymentfieldCount++;
								 }	*/							
							break;							
							case 'multiselect':
							...
							case 'select':
								...
								for($c = 0; $c < count($choices); $c++) {
								   ...
								   if($choices[$c]['value'] == $inputs && $inputs != "")
								    {
										$item_custom['FieldName'] = $field["label"] ; //. ': ' . $choices[$c]['value']
										$item_custom['FieldValue'] = $choices[$c]['value'];
										$item_custom['FieldId'] = $id;
										$select_array[] = $item_custom;
										$this->inc++;
							...
							//echo $this->inc;
								if ((strtolower($field['label']) == 'payment method' ||      strtolower($new_arry['label']) == 'select payment type'  || 
							     strtolower($new_arry['label']) == 'select payment method') && $this->inc == 0)
								 {
									$this->paymentfieldCount++;
								 }	
							...
							case 'radio':								
								...
								$item_custom['FieldId'] = $id;
								//echo strtolower($field['label']);echo $str;
								if ((strtolower($field['label']) == 'payment method' || strtolower($new_arry['label']) == 'select payment type'  || 
							     strtolower($new_arry['label']) == 'select payment method') && $str == "")
								 {
									$this->paymentfieldCount++;
								 }
								//echo $this->paymentfieldCount;
							break;
							case 'html':
							case 'section':
							case 'page':
						...
						elseif($field['type'] == 'select') {						
							if(count($select_array)) {
								foreach($select_array as $cbfield)	{
									$this->customfields[] = $cbfield;
								}							
							}							
		...
		if (!RGFormsModel::is_field_hidden($form, $field, array())) {
			...
			switch ($field["inputType"]) {
				...
				case 'calculation':
					...
					if($qtycalculation > 0 && $pricecalculation >= 0) { 
						$isProduct = true;						
